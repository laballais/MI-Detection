...
class AttU_Net_LalaSan(nn.Module):
    """
    Attention Unet implementation
    Paper: https://arxiv.org/abs/1804.03999
    """
    def __init__(self, img_ch=3, output_ch=1):
        super(AttU_Net_LalaSan, self).__init__()
        ...
        #default kernel=3, stride=1, padding=1
        self.Conv1 = conv_block(img_ch, filters[0])     # 128x128   224x224
        self.Conv2 = conv_block(filters[0], filters[1]) # 64x64     112x112
        self.Conv3 = conv_block(filters[1], filters[2]) # 32x32     56x56
...
class AttU_Net(nn.Module):
    ...
    def __init__(self, img_ch=3, output_ch=1):
        ...
        # Input: 128 x 128  224x224

        self.Maxpool1 = nn.MaxPool2d(kernel_size=2, stride=2)   # 64x64 112x112
        self.Maxpool2 = nn.MaxPool2d(kernel_size=2, stride=2)   # 32x32 56x56
        self.Maxpool3 = nn.MaxPool2d(kernel_size=2, stride=2)   # 16x16 28x28
        self.Maxpool4 = nn.MaxPool2d(kernel_size=2, stride=2)   # 8x8   14x14
        self.Maxpool5 = nn.MaxPool2d(kernel_size=2, stride=2)   # 4x4   7x7

        #default kernel=3, stride=1, padding=1
        self.Conv1 = conv_block(img_ch, filters[0])     # 128x128   224x224
        self.Conv2 = conv_block(filters[0], filters[1]) # 64x64     112x112
        self.Conv3 = conv_block(filters[1], filters[2]) # 32x32     56x56
        self.Conv4 = conv_block(filters[2], filters[3]) # 16x16     28x28
        self.Conv5 = conv_block(filters[3], filters[4]) # 8x8       14x14
        self.Conv6 = conv_block(filters[4], filters[5]) # 4x4       7x7
    ...
    def forward(self, x):
        ...
        d6 = self.Up6(e6)
        # print("upsampled: " + str(d6.shape))
        x5 = self.Att6(g=d6, x=e5)
        d6 = torch.cat((x5, d6), dim=1)
        d6 = self.Up_conv6(d6)
        # print("up convolved:" + str(d6.shape))

        # print(x5.shape)
        d5 = self.Up5(e5)
        # print("upsampled: " + str(d5.shape))
        x4 = self.Att5(g=d5, x=e4)
        d5 = torch.cat((x4, d5), dim=1)
        d5 = self.Up_conv5(d5)
        # print("up convolved:" + str(d5.shape))

        d4 = self.Up4(d5)
        # print("upsampled: " + str(d4.shape))
        x3 = self.Att4(g=d4, x=e3)
        d4 = torch.cat((x3, d4), dim=1)
        d4 = self.Up_conv4(d4)
        # print("up convolved:" + str(d4.shape))

        d3 = self.Up3(d4)
        # print("upsampled: " + str(d3.shape))
        x2 = self.Att3(g=d3, x=e2)
        d3 = torch.cat((x2, d3), dim=1)
        d3 = self.Up_conv3(d3)
        # print("up convolved:" + str(d3.shape))

        d2 = self.Up2(d3)
        # print("upsampled: " + str(d2.shape))
        x1 = self.Att2(g=d2, x=e1)
        d2 = torch.cat((x1, d2), dim=1)
        d2 = self.Up_conv2(d2)
        # print("up convolved:" + str(d2.shape))
